...
<%
	...
	if (rtnParams != null) {
		videoId = rtnParams.get("videoId") == null? "": rtnParams.get("videoId");
		...
		videoCtgr = rtnParams.get("videoCtgr") == null? "": rtnParams.get("videoCtgr");
	...
	<script>
		...
		$(function(){
			...
			if (!sltd.equals("")) {
				...
				$("#videoCtgr").val("<%=videoDetail.get("videoCtgr")%>").prop("selected", true);
			}
		});
...
<body>
	...
	<!-- 비디오 입력 폼 -->
	<section id="videoModifying">
		<form name="videoModiFrm" action="<%=contextPath%>/jiutopia/videoModi.do" method="post">
			<p>
				<input type="hidden" name="videoId" value="<%=videoDetail.get("videoId")%>">
			</p>
			<p>
				제목 : <input type="text" 
							  name="videoTitle"
							  value="<%=videoDetail.get("videoTitle")%>"
			...
			<p>
				URL : <input type="text" 
							 name="videoUrl"
							 value="<%=videoDetail.get("videoUrl")%>"
							 placeHolder="이곳에 URL을 입력하세요." 
							 style="width: 400px;">
			</p>
			<p> 분류 : <select id="videoCtgr" name="videoCtgr">
							<option value="SPT">스포츠 영상</option>
							<option value="DEF">호신술 영상</option>
					   </select>
			</p>
			<p>
				내용 : <textarea name="videoCont" 
								 rows="17" 
								 cols="140"
								 placeHolder="이곳에 내용을 입력하세요."><%=videoDetail.get("videoCont")%>
					   </textarea>
			</p>
			<input type="submit" name="modiBtn" value="수정" class="button">
			<input type="button" name="cancelBtn" value="취소" class="button" onClick="postCancel();">
		</form>
	</section>
	...
	<footer id="footer">
		<p>
			<b>“ 주짓수는 작고 약한 사람이 자신을 보호할 수 있는 유일한 호신술이다. ” 엘리오 그레이시(1913 – 2009)</b>
		</p>
	</footer>
</body>
